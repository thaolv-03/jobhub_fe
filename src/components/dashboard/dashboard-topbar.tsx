...
export function DashboardTopbar({
  ...
  return (
    <header className="sticky top-0 z-20 flex h-16 items-center gap-4 border-b bg-background/80 px-4 backdrop-blur">
      ...
      <div className="ml-auto flex flex-1 items-center justify-end gap-2 md:gap-3">
        {searchPlaceholder ? (
          <div className="hidden max-w-xs flex-1 md:block">
            <Input placeholder={searchPlaceholder} className="h-9" />
          </div>
        ) : null}
        {primaryAction}
        {rightActions}
