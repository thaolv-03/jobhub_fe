...
import { Building, MapPin, DollarSign, Briefcase, Bookmark, Share2, CalendarClock } from "lucide-react";
      ...
      <main className="flex-1">
        <div className="container max-w-5xl mx-auto py-8">
          <div className="relative w-full h-48 md:h-64 rounded-xl overflow-hidden mb-8">
              {bannerImage && <Image src={bannerImage.imageUrl} alt="Job Banner" fill style={{objectFit: 'cover'}} data-ai-hint={bannerImage.imageHint} />}
          </div>

          <div className="bg-card border rounded-xl p-6 md:p-8 -mt-20 relative z-10 max-w-4xl mx-auto shadow-lg">
              <div className="flex flex-col md:flex-row items-start gap-6">
                  {companyLogo && <Image src={companyLogo.imageUrl} alt={`${jobData.company} logo`} width={80} height={80} className="rounded-md border p-1" data-ai-hint={companyLogo.imageHint} />}
                  <div className="flex-1">
                      ...
                      <div className="flex flex-wrap gap-x-6 gap-y-2 mt-2 text-muted-foreground">
                          ...
                          {jobData.deadline && <p className="flex items-center gap-2"><CalendarClock className="w-5 h-5 text-primary" /> Hạn nộp: {format(jobData.deadline, "dd/MM/yyyy")}</p>}
                      </div>
                  </div>
